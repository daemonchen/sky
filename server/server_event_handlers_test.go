...
func TestServerStreamUpdateEventsAreDeduped(t *testing.T) {
	runTestServer(func(s *Server) {
		...
		// Send 3 events (2 distinct) in one request.
		resp, _ := sendTestHttpRequest("PATCH", "http://localhost:8586/tables/foo/events", "application/json", `{"id":"xyz","timestamp":"2012-01-01T02:00:00.000000111Z","data":{"bar":"myValue", "baz":12}}{"id":"xyz","timestamp":"2012-01-01T02:00:00.000000222Z","data":{"bar":"myValue", "baz":12}}{"id":"xyz","timestamp":"2012-01-01T03:00:00Z","data":{"bar": "myValue2"}}`)
		assertResponse(t, resp, 200, `{"events_written":3}`, "PATCH /tables/:name/events failed.")
