...
func BenchmarkInsertEvent0001(b *testing.B) {
	e := &Event{Data: map[int64]interface{}{1: 100}}
...
func benchmarkInsertEvent(b *testing.B, e *Event, eventsPerObject int, fn func()) {
...
func benchmarkInsertEvents(b *testing.B, eventsPerObject int) {
	...
	for i := 0; i < eventsPerObject; i++ {
		events = append(events, &Event{Timestamp: timestamp.Add(time.Duration(i) * time.Second), Data: map[int64]interface{}{1: 100}})
	}
